...
public class UsersAPIController {
		...
		public UserBoundary insertUserDataToDatabase (@RequestBody NewUserDetails user) {
			...
			return new UserBoundary(new userIdBoundary("2021b.twins",user.getEmail()),user.getRole(),user.getUsername(),user.getAvatar());
		}
		...
		@RequestMapping(
			...
			public UserBoundary loginUserAndRetrieve (@PathVariable("userSpace") String space,@PathVariable("userEmail") String email) {
				...
				return new UserBoundary(new userIdBoundary(space,email),"Team Leader","Tal Goldengoren","T");
			}
